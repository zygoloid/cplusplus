...
\item has no element of the set \tcode{M(S)} of types (defined below)
...
\tcode{M(X)} is defined as follows:
\begin{itemize}
\item If \placeholder{X} is a non-union class type, the set \placeholder{M(X)} is
empty if \placeholder{X} has no (possibly inherited (Clause~\ref{class.derived}))
non-static data members;
otherwise, it consists of the type of the first non-static data member
of \placeholder{X} (where said member may be an anonymous union), \placeholder{X0},
and the elements of \placeholder{M(X0)}.

\item If \placeholder{X} is a union type, the set \placeholder{M(X)} is
the union of all \placeholder{M(Ui)} and the set containing all \placeholder{Ui},
where each \placeholder{Ui} is the type of the $i^{th}$ non-static data member
of \placeholder{X}.

\item If \placeholder{X} is a non-class type, the set \placeholder{M(X)} is empty.
\end{itemize}

\enternote \placeholder{M(X)} is the set of the types of all non-base-class subobjects
that are guaranteed in a standard-layout class to be at a zero offset
in \placeholder{X}. \exitnote
