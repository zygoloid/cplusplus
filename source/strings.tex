...
\tcode{traits::compare(data(), s, n) == 0}.
\end{itemdescr}
...
\tcode{allocator_traits} \tcode{<Allocator>::allocate}.\footnote{}
\end{itemdescr}
...
\effects
Equivalent to:
\tcode{return append(basic_string_view<charT, traits>(str).substr(pos, n));}
\end{itemdescr}
...
\end{codeblock}
has effects equivalent to:
\tcode{return \placeholder{F}(basic_string_view<charT, traits>(addressof(c), 1), pos);}
\end{itemize}
...
and \tcode{size_} with \tcode{traits::length(str)}.
...
\end{codeblock}
has effects equivalent to: \tcode{return \placeholder{F}(basic_string_view(\&c, 1), pos);}
\end{itemize}
