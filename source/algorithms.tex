...
\pnum
\effects Copies elements in the range \range{first}{last} into the range \range{result}{result + (last - first)} starting from \tcode{first} and proceeding to \tcode{last}.
For each non-negative integer $n < \tcode{(last - first)}$, performs \tcode{*(result + $n$) = *(first + $n$)}.

\pnum
\returns
\begin{itemize}
\item
\tcode{result + (last - first)} for the overload in namespace \tcode{std}, or
\item
\tcode{\{last, result + (last - first)\}} for the overloads in
...
\item \tcode{\{last, result - (last - first)\}}
  for the overloads in namespace \tcode{ranges}.
\end{itemize}
...
\returns
\begin{itemize}
\item \tcode{result - (last - first)} for the overload in namespace \tcode{std}, or
\item \tcode{\{last, result - (last - first)\}} for the overloads
...
\tcode{ranges::iter_swap} for the overloads in namespace \tcode{ranges},
...
of
\tcode{[first, last)},
\tcode{bool(comp(e, value))} shall imply \tcode{!bool(comp(value, e))}
for the overloads in namespace \tcode{std}.
...
\tcode{bool(invoke(comp, invoke(proj, a[$\left \lfloor{\frac{i - 1}{2}}\right \rfloor$]), invoke(proj, a[$i$])))}
is \tcode{false}.
\item \tcode{*a}
may be removed by
\tcode{pop_heap},
