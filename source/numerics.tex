...
\tcode{pow(0,0)}
is \impldef{return value of \tcode{pow(0,0)}}.
\end{itemdescr}
...
\pnum\effects Constructs a \tcode{random_device}
 ...
 The semantics and default argument of the \tcode{token}
 parameter are
\impldef{\idxcode{random_device}!semantics and default
argument of constructor parameter}.%
...
\pnum
\throws A value of an
\impldef{\idxcode{random_device}!type of exception thrown
if initialization fails} type
...
\pnum\returns A non-deterministic random value,
 ...
 inclusive.
 It is
\impldef{\idxcode{random_device}!mechanism for generating random numbers}
how these values are generated.

\pnum
\throws A value of an
\impldef{\idxcode{random_device}!type of exception thrown
if random number cannot be obtained} type
 derived from \tcode{exception}
...
are \impldef{algorithms used by
...
\tcode{rand}. It is \impldef{whether \tcode{rand} may introduce data races}
